...
hand_eval_t build_hand_from_match(deck_t * hand,
  ...
  for (int k = n; k < 5; k++){
	  if (j < idx){
		  ans.cards[k] = hand->cards[j];
		  j++;
	  } else {
	  ans.cards[k] = hand->cards[k];
	  }
  }
  return ans;
}
...
int compare_hands(deck_t * hand1, deck_t * hand2) {
	...
	hand_eval_t hand1_fin = evaluate_hand(hand1);
	hand_eval_t hand2_fin = evaluate_hand(hand2);
	
	if (hand1_fin.ranking < hand2_fin.ranking){
		return 1;
	} else if (hand1_fin.ranking > hand2_fin.ranking){
		return -1;
	} else {
		for (int i = 0; i < 5; i++){
			if (hand1_fin.cards[i]->value > hand2_fin.cards[i]->value){
				return 1;
			} else if (hand1_fin.cards[i]->value < hand2_fin.cards[i]->value){
				return -1;
			}
		}
		return 0;
...
unsigned * get_match_counts(deck_t * hand) {
	int counter = 1;
	int index = 0;
	unsigned *match_count = malloc(hand->n_cards * sizeof(unsigned));
	for (size_t i = 0; i < hand->n_cards; i++) {
		if (i + 1 < hand->n_cards && hand->cards[i]->value == hand->cards[i + 1]->value) {
			counter++;
		} else {
			for (int j = i; j >= index; j--) {
				match_count[j] = counter;
			}
			counter = 1;
			index = i + 1;
		}
